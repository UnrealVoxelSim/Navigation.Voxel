...
namespace UnrealVoxelSim::Navigation::Voxel
{
	namespace
	{
		...
		Location(const std::int32_t x, const std::int32_t y, const std::int32_t z = 1)
		{
			...
			for (auto _ : state)
			{
				...
				while (planner.GetPlanState(Navigation::Api::PlanRequestId{1}) == Navigation::Api::PlanState::Pending)
				{
					...
					maximumTickMilliseconds =
						std::max(maximumTickMilliseconds,
		...
		void BatchedReachabilityFiveHundredDestinations(benchmark::State& state)
		{
			...
			for (auto _ : state)
			{
				static_cast<void>(_);
				Planner planner{terrain, profiles};
				if (!planner.BeginReachability(
						{Navigation::Api::ReachabilityRequestId{1}, profiles[0].Id, Location(-200, 0), destinations}))
